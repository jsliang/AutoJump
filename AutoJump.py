...
def remove_nonexisting_entries(view):
    ...
    """
    update_autojump_database = load_setting(
        view, "update_autojump_database", True)
...
def load_recent_files(view):
    ...
    session_dir = os.path.join(os.path.dirname(
        sublime.packages_path()), "Settings")
    ...
    def traverse_subfolder(self, picked):
        ...
        if not os.path.exists(self.picked_folder):
            sublime.error_message(
                "Folder %s does not exist." % self.picked_folder)
            remove_nonexisting_entries()
            return

        view = self.window.active_view()
        exclude_filepath_filters = load_setting(
            view, "exclude_filepath_filters", None)
        ...
        if len(self.file_list) > 0:
            ...
            sublime.error_message(
                "%s is an empty folder." % self.picked_folder)
